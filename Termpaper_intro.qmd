...
## i. Huskarakterisika
```{r}
mod1 <- "price ~ bedrooms + bathrooms + sqft_living + sqft_lot + floors + waterfront + view + condition + grade + sqft_above + sqft_basement + yr_built + yr_renovated + sqft_living15 + sqft_lot15"
```
## ii. Huskarakteristika + distanse til cbd + tracts_var
```{r}
mod2 <- "price ~ bedrooms + bathrooms + sqft_living + sqft_lot + floors + waterfront + view + condition + grade + sqft_above + sqft_basement + yr_built + yr_renovated + sqft_living15 + sqft_lot15 + dist_cbd_km + linguist_2 + poverty_pe"
```
## iii. Huskarakteristika + distanse til cbd + EHD
```{r}
mod3 <- "price ~ bedrooms + bathrooms + sqft_living + sqft_lot + floors + waterfront + view + condition + grade + sqft_above + sqft_basement + yr_built + yr_renovated + sqft_living15 + sqft_lot15 + dist_cbd_km + EHD_percen + low + high + year_month"
```
