...
Morans I verdien til K3 er på 0,398 og K10 sin er på 0,350. Når vi skulle regne ut Morgans I valgte vi å bruke price og sqft_living. Vi sammenlignet sqft_living og bedrooms, de så ganske like ut på kartet, men hadde forskjellige verdier på morans I. Verdien er finere med sqft_living fordi det er et tydligere mønster på at når sqft_living øker så lir det dyrere. 

Verdier som er positive taler for klynging. Siden verdiene er svakt positive så viser de til litt klynging.
...
## i. Huskarakteristika
```{r}
mod1 <- "price ~ bedrooms + I(bedrooms^2) + bathrooms + sqft_living + sqft_lot + sqft_above + floors + grade + yr_built + yr_renovated + waterfront + condition + view + year_month"
```
...
```{r}
mod2 <- "price ~ bedrooms + I(bedrooms^2) + bathrooms + sqft_living + sqft_lot + sqft_above + floors + grade + yr_built + yr_renovated + waterfront + condition + view + year_month + dist_cbd_km + linguist_2 + poverty_pe + POC_percen + unemploy_2 + sen_pop_pe + facilities + wastewater + traffic_pe + diesel + superfund + transporta + housing_pe + ozone + PM25 + toxic_rele + hazardous_ + lead_perce + socio_perc"
```
...
```{r}
mod3 <- "price ~ bedrooms + I(bedrooms^2) + bathrooms + sqft_living + sqft_lot + sqft_above + floors + grade + yr_built + yr_renovated + waterfront + condition + view + dist_cbd_km + EHD_percen + low + high + year_month"
```
...
```{r}
hedon2_seed <- lm(mod2, data = kc_house_data_6666)
```
...
```{r}
lm.morantest(hedon2_seed, kc_house_data_6666_W)
lm.morantest(hedon2_seed, kc_house_data_6666_W10)
```
...
```{r}
kc_lagrange_3 <- lm.LMtests(hedon2_seed, kc_house_data_6666_W, 
               test = "all")
kc_lagrange_3
```

```{r}
kc_lagrange_10 <- lm.LMtests(hedon2_seed, kc_house_data_6666_W10, 
               test = "all")
kc_lagrange_10
```
...
```{r}
SDEM_seed <- errorsarlm(mod2, data = kc_house_data_6666, listw = kc_house_data_6666_W, Durbin = as.formula(~ bedrooms + I(bedrooms^2) + bathrooms + sqft_living + sqft_lot + sqft_above + floors + grade + yr_built + yr_renovated + waterfront + condition + view + year_month + dist_cbd_km + linguist_2 + poverty_pe + POC_percen + unemploy_2 + sen_pop_pe + facilities + wastewater + traffic_pe + diesel + superfund + transporta + housing_pe + ozone + PM25 + toxic_rele + hazardous_ + lead_perce + socio_perc))
```


```{r}
SLX_seed <- lmSLX(mod2, data = kc_house_data_6666, listw = kc_house_data_6666_W, Durbin = as.formula(~ bedrooms + I(bedrooms^2) + bathrooms + sqft_living + sqft_lot + sqft_above + floors + grade + yr_built + yr_renovated + waterfront + condition + view + year_month + dist_cbd_km + linguist_2 + poverty_pe + POC_percen + unemploy_2 + sen_pop_pe + facilities + wastewater + traffic_pe + diesel + superfund + transporta + housing_pe + ozone + PM25 + toxic_rele + hazardous_ + lead_perce + socio_perc))
```

```{r}
SEM_seed <- errorsarlm(mod2, data = kc_house_data_6666,
