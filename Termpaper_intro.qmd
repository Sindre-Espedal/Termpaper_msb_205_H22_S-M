...
```{r}
SDEM_seed <- errorsarlm(mod2, data = kc_house_data_6666, listw = kc_house_data_6666_W, Durbin = TRUE)
```

```{r}
SLX_seed <- lmSLX(mod2, data = kc_house_data_6666, listw = kc_house_data_6666_W, Durbin = TRUE)
```
...
```{r}
huxreg("SEM" = SEM_seed, "OLS" = hedon2_seed, 
    error_format = "[{statistic}]",
    note = "{stars}. T statistic in brackets.")
```
...
```{r}
moran.plot(residuals(hedon2_seed), listw = kc_house_data_6666_W10)
```
